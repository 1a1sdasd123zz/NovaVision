...
public partial class FrmIRAYPLEFrameGrabber : Form
{
    ...
    public FrmIRAYPLEFrameGrabber(FrameGrabberConfigData paramValues)
        : this(paramValues, flag: false)
    {
    }

    public FrmIRAYPLEFrameGrabber(FrameGrabberConfigData paramValues, bool flag)
    {
        InitializeComponent();
        enterFlag = flag;
    ...
    private void FrmFrameGrabberSetting_Load(object sender, EventArgs e)
    {
        ...
        tsslStatus.Text = "正在枚举IRAYPLECL卡设备";
        if (FrameGrabberOperator.hardwareDeployments[19].state != 262144)
        {
        ...
        {
            if (FrameGrabberOperator.dicVendorNameKey.ContainsKey("IRAYPLECL"))
            {
                ListBox.ObjectCollection items3 = listDevice.Items;
                object[] items4 = FrameGrabberOperator.dicVendorNameKey["IRAYPLECL"].ToArray();
        ...
        cbCameraSerial.Invoke((MethodInvoker)delegate
        {
            ...
            if (cbCameraSerial.Items.Count > 0)
            {
                ...
                if (!enterFlag)
                {
                    m_ParamValues = FrameGrabberOperator.FindDeviceConfigByVendorKey("IRAYPLECL," + m_SerialCamera);
                }
            ...
            {
                timeSpan = DateTime.Now - now;
                tsslStatus.Text = "未找到IRAYPLECL相机";
